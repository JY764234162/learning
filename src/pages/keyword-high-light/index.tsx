...
export const HighLight: FC<HighLightProps> = ({ words, text }) => {
  ...
  const render = useMemo(() => {
    return offWords.map((item, index) => {
      if (item.highlight) {
        const type =
          KEYWORD_TYPES.find((t) => t.value === item.meta?.type) ||
          KEYWORD_TYPES[4];
        return (
          <Tooltip
            key={index}
            ...
            <div>
              ...
              <div style={{ marginBottom: 16 }}>
                {keywords.map((item, index) => {
                  const type =
                    KEYWORD_TYPES.find((t) => t.value === item.type) ||
                    KEYWORD_TYPES[4];
                  return (
                    <span
                      key={item.keywordTag}
...
export default KeywordHighlightDemo;
